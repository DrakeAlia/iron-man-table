...
export const useSupabaseTables = () => {
  ...
  const detectForeignKeys = (
    tableName: string,
    columns: Column[]
  ): { name: string; type: string; foreignKey?: string }[] => {
    ...
    const fetchTables = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setError(
            "Unable to fetch tables. Tables will need to be configured manually."
          );
        }
      } catch (err) {
